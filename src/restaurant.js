...
const createMenu = (myMenu) => {
  ...
  const checkCalculator = () => {
    const fullMenu = restaurant.fetchMenu();
    const foodMenu = fullMenu.food;
    const foodKeys = Object.keys(foodMenu);
    const drinkMenu = fullMenu.drink;
    const drinkKeys = Object.keys(drinkMenu);
    const orderedItems = restaurant.consumption;
    console.log(orderedItems);
    let total = 0;
    for (let index = 0; index < orderedItems.length; index += 1) {
      let currentKey = orderedItems[index];
      if (foodKeys.includes(currentKey)) {
        total += parseFloat(foodMenu[currentKey]);
      }
      if (drinkKeys.includes(currentKey)) {
        total += parseFloat(drinkMenu[currentKey]);      
      }
    }
    total *= 1.1;
    console.log(total);
  ...
  restaurant.pay = checkCalculator;
  return restaurant;
};
